...
int numOfActiveProc = 10; // Broj procesa.
System* sys;
...
void f(System* sys) {
	...
	GLOBAL_LOCK.lock();
	++br;
	VirtualAddress startShared;
	if (!deleted) {
		startShared = ss(SEG_SIZE + br + 1);
	...
	for (VirtualAddress vaddr = 0UL; vaddr < PAGE_SIZE * SEG_SIZE; ++vaddr) {
		...
		if (kol == k) {
			...
			if (status == Status::PAGE_FAULT) {
				Status s = p->pageFault(startShared);
				if (deleted) {
					assert(s != Status::OK);
					GLOBAL_LOCK.unlock();
					continue;
			...
			*paddr = counter;
			if (k == 10 && !deleted) {
				deleted = true;
				assert(p->deleteSharedSegment("radi") != Status::TRAP);
				std::cout << "DELETING SHARED SEG!" << std::endl;
			}
			//assert(p->disconnectSharedSegment("radi") != Status::TRAP);
		}
